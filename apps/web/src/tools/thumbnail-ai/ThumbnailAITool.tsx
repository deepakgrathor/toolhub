...
export default function ThumbnailAITool({
  ...
  useEffect(() => {
    ...
    fetch("/api/tools/thumbnail-ai/history?page=1")
      .then((r) => r.json())
      .then((data: { thumbnails?: HistoryItem[] }) => {
        console.log("Thumbnail history response:", data);
        console.log("Thumbnails count:", data.thumbnails?.length ?? 0);
        setHistoryItems(data.thumbnails ?? []);
      })
      .catch((err) => {
  ...
  async function generate() {
    ...
    if (balance < totalCredits) {
      openPaywall(thumbnailAIConfig.name, totalCredits);
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          (err as { detail?: string }).detail ?? "generation_failed",
        );
      }
        ...
        <div className="lg:w-[45%] lg:border-r border-border p-4 md:p-6 space-y-5 overflow-y-auto">
          ...
          <PresetSelector
            ...
            onPresetLoad={(inputs) => {
              ...
              if (inp.colorTheme) setColorTheme(inp.colorTheme);
              if (inp.faceMode)
                setFaceMode(inp.faceMode as "ai" | "own" | "none");
              if (inp.gender) setGender(inp.gender as "male" | "female");
              ...
              <RevealSection show={platform !== ""}>
                <div className="space-y-2 mt-1">
                  <p className="text-xs text-muted-foreground">
                    Size — auto-selected for{" "}
                    {PLATFORM_LABELS[platform] ?? platform}. Change if needed.
                  </p>
                    ...
                    {RATIO_CARDS.map((card) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <p
                            className={`text-xs font-semibold mt-1 ${isActive ? "text-accent" : "text-foreground"}`}
                          >
                            {card.ratio}
                          </p>
            ...
            <RevealSection show={platform !== ""}>
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    What is your video/post about? More detail = better
                    thumbnail
                  </p>
                  <textarea
                    value={topic}
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">
                    Niche + mood auto-picked if skipped
                  </span>
                </div>
            ...
            <RevealSection show={topic.length >= 10}>
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  >
                    <Sparkles
                      className={`h-5 w-5 mx-auto mb-1.5 ${faceMode === "ai" ? "text-accent" : "text-muted-foreground"}`}
                    />
                  ...
                  >
                    <User
                      className={`h-5 w-5 mx-auto mb-1.5 ${faceMode === "own" ? "text-accent" : "text-muted-foreground"}`}
                    />
                    <p className="text-xs font-medium text-foreground">
                      My Photo
                    </p>
                    <p className="text-[10px] text-muted-foreground mt-0.5">
                      {baseCost + faceAddonCost} cr
                    </p>
                  </button>
                  ...
                  >
                    <ImageIcon
                      className={`h-5 w-5 mx-auto mb-1.5 ${faceMode === "none" ? "text-accent" : "text-muted-foreground"}`}
                    />
                    <p className="text-xs font-medium text-foreground">
                      No Face
                    </p>
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      {isGenerating ? (
                        ...
                        <>
                          <Coins className="h-4 w-4" />
                          Generate Thumbnail — {totalCredits} credits
                        </>
                      )}
                    </button>
