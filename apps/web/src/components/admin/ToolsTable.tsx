...
export function ToolsTable({ initialTools }: { initialTools: AdminToolRow[] }) {
  ...
  // Separate draft state for the credits input so blur-to-save works
  ...
  function handleCreditsBlur(slug: string) {
    const draft = creditDrafts.get(slug) ?? 0;
    const current = rows.get(slug)?.creditCost ?? 0;
    if (draft !== current) {
      setRows((prev) => {
        const m = new Map(prev);
        const row = m.get(slug);
          ...
          {initialTools.map((tool, i) => {
            ...
            return (
              ...
              >
                ...
                {/* Toggle switch — save on click */}
                <td className="px-4 py-3">
                  <button
                    onClick={() => handleToggle(tool.slug)}
                    aria-label={state.isActive ? "Disable" : "Enable"}
                    className={cn(
                      ...
                      state.isActive ? "bg-[#7c3aed]" : "bg-border"
                    )}
                  >
                    <span
                      className={cn(
                        "inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition-transform",
                        state.isActive ? "translate-x-4" : "translate-x-0.5"
                      )}
                    />
                  </button>
                </td>
