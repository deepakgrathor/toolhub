...
import {
  MapPin, Quote, AlertCircle, ChevronRight, Zap, CheckCircle,
} from 'lucide-react'
...
function getInitials(name: string): string {
  const parts = name.replace(/^Adv\.\s*/, '').trim().split(' ')
          ...
          >
            ...
            <div className="rounded-xl border border-border bg-card p-6 sm:p-8">
              <div className="flex flex-col sm:flex-row sm:items-start gap-6">
                ...
                <div className="flex items-start gap-4">
                  <div className={cn('h-12 w-12 rounded-full flex items-center justify-center shrink-0 text-base font-bold', c.bg, c.text)}>
                    {getInitials(active.name)}
                  </div>
                  <div>
                    <h3 className="text-xl font-bold text-foreground">{active.name}</h3>
                    <p className="text-sm text-muted-foreground mt-0.5">{active.role}</p>
                    <p className="text-sm text-muted-foreground flex items-center gap-1 mt-0.5">
                      <MapPin size={13} className="shrink-0" />
                      {active.city}
                    </p>
                    <span className={cn('inline-block text-xs font-medium px-2.5 py-0.5 rounded-full mt-2', c.badge)}>
                      {active.kitBadge}
                    </span>
                  </div>
                </div>
                ...
                {active.steps.map((step, i) => (
                  <div key={i} className="flex items-start gap-0">
                    <div className="relative px-4 py-3 flex flex-col">
                      <div className={cn('w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0', c.bg, c.text)}>
                        {i + 1}
                      </div>
                      <p className="text-sm font-medium text-foreground mt-2 max-w-[140px]">{step.label}</p>
                      <p className="text-xs text-muted-foreground mt-0.5 max-w-[140px]">{step.detail}</p>
                    </div>
            ...
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
              {active.tools.map(tool => (
                <div key={tool.slug} className="rounded-xl border border-border bg-card p-4 space-y-2">
                  <div className="flex items-center gap-2">
                    <DynamicIcon name={tool.icon} size={18} className={c.text} />
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">{tool.what}</p>
            ...
            <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-4">
              {active.stats.map((stat, i) => (
                <div key={i} className={cn('rounded-xl border p-4 text-center space-y-1', c.border, c.bg)}>
                  <p className="text-xs text-muted-foreground font-medium">{stat.label}</p>
                  <p className="text-sm line-through text-muted-foreground/60 mt-1">{stat.before}</p>
                  <p className={cn('text-xl font-bold mt-0.5', c.text)}>{stat.after}</p>
                </div>
              ))}
            </div>
